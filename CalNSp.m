function data = CalNSp(dt, N, t1)
% Here is a NS demo where dt and N is parameters
% return the residual at time X.Y s
...
for t = dt:dt:t1
    ...
    if ~mod(t, 0.1)
        k = k+1;
        err = X - [U1(U.Node(:, 1), U.Node(:, 2)); U2(U.Node(:, 1), U.Node(:, 2)); pe(P.Node(:, 1), P.Node(:, 2))];
        data.err{k} = err;
    end
end
end