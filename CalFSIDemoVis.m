...
%% Initial Mesh
TF = RecMesh([8, 4], [L, RF], [0, 0]);
TS = RecMesh([8, 2], [L, RS], [0, RF]);
UF = P2Fespace(TF);
PF = P1Fespace(TF);
US = P2Fespace(TS);
PS = P1Fespace(TS);
...
for t = 0:dt:runtime
    ...
    %box off; set(gca, 'XTick', [], 'YTick', [], 'xcolor', 'white', 'ycolor', 'white');
    axis off
    saveas(gcf, ['abc', num2str(npic) ,'.jpg'])
    npic = npic + 1;
    %title({['t = ', num2str(t), 's']});
    %}
    triplot(TF.Tri, TF0.Node(:,1)+A*(TF.Node(:, 1)-TF0.Node(:,1)), TF0.Node(:,2)+A*(TF.Node(:, 2)-TF0.Node(:,2)))
    axis equal
    axis off
    pause;
end
